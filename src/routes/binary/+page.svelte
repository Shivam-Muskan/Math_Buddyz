...
<section id="binaryCalculator" class="py-5 px-2">
	<div class="mx-auto grid max-w-7xl justify-items-center space-y-4">
		<div class="form-control">
			...
			>
				<div class="dropdown">
					<button tabindex="0" class="btn md:rounded-r-none bg-indigo-600 hover:bg-indigo-500 border-none text-gray-100">
						From: {CNSCalculator[selections['from']].name}
				...
				<input
					...
					class="rounded-lg px-2 py-2 bg-gray-50 border-indigo-800 md:input md:bg-gray-50 md:border-indigo-800"
				/>
				<div class="dropdown dropdown-bottom dropdown-end">
					<button tabindex="0" class="btn md:rounded-l-none bg-indigo-600 hover:bg-indigo-500 border-none text-gray-100 justify-items-center">
						To: {CNSCalculator[selections['to']].name}
						<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 ml-2">
							<path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
						</svg>
		...
		<button
			class="btn mt-4 bg-indigo-600 hover:bg-indigo-500 text-gray-100 border-indigo-600 hover:border-indigo-500"
			on:click={() => {
				handleCalculations();
			}}
